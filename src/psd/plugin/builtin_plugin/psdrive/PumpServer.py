...
from tango.server import Device, command, attribute, pipe, device_property, run
import time
import queue
...
class PumpServer(Device):
    ...
    def init_device(self):
        ...
        self._img_R = np.ones((10,10), dtype=np.uint8)
        self._img_G = np.ones((10,10), dtype=np.uint8)
        self._img_B = np.ones((10,10), dtype=np.uint8)
        # self._img_dim = (10,10,3)

        # self.serialinstance = serial.Serial()
        self._port = ''
        self.count = 0
        self.__sequence = np.zeros(16, dtype=int)

        self.__availableDeviceId = np.array([])

        self._statusbytes = np.full(16, 255, dtype=np.uint8)
        self._syringePos = np.full(16, -1, dtype=int)
        self._valvePos = np.full(16, -1, dtype=int)

        self._syringeStatus = np.full(16, 512, dtype=int)
        self._valveStatus = np.full(16, 512, dtype=int)

        # 0 for MVP; 1 for syringe pump;
        self._deviceType = np.full(16, -1, dtype=int)
    ...
    @attribute(dtype=(int,), access=AttrWriteType.READ, display_level=DispLevel.OPERATOR,
               max_dim_x=16)
    def syringePos(self):
    ...
    @attribute(dtype=((np.uint8,),),label="webcam image: R channel", access=AttrWriteType.READ_WRITE, max_dim_x=3000, max_dim_y=3000, fset="set_webcamimg_R")
    def webCamImgR(self):
        return self._img_R

    def set_webcamimg_R(self, img):
        self._img_R = img

    @attribute(dtype=((np.uint8,),),label="webcam image: G channel", access=AttrWriteType.READ_WRITE, max_dim_x=3000, max_dim_y=3000,fset="set_webcamimg_G")
    def webCamImgG(self):
        return self._img_G

    def set_webcamimg_G(self, img):
        self._img_G = img

    @attribute(dtype=((np.uint8,),),label="webcam image: B channel", access=AttrWriteType.READ_WRITE, max_dim_x=3000, max_dim_y=3000,fset="set_webcamimg_B")
    def webCamImgB(self):
        return self._img_B

    def set_webcamimg_B(self, img):
        self._img_B = img        
            ...
            for i in range(
                ...
                try:
                    ...
                    print(
                        "ScanDevices: Unexpected error during transmission: device answered, but now not anymore!")
                    continue
            self.__availableDeviceId = np.array(__availDev, dtype=int)
