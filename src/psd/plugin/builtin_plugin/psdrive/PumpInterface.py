...
class PumpClient(object):
    ...
    def fromFile(cls, filename: str):
        ...
        if not server['database']:
            ...
            name = f"{server['host']}:{server['port']}/{server['tangoname']}"
    ...
    @property
    def webCamImg(self):
        return np.concatenate((self._device.webCamImgR[:,:,np.newaxis], self._device.webCamImgG[:,:,np.newaxis], self._device.webCamImgB[:,:,np.newaxis]),axis=2)

    @webCamImg.setter
    def webCamImg(self, img):
        self._device.webCamImgR, self._device.webCamImgG, self._device.webCamImgB = img[:,:,0],img[:,:,1],img[:,:,2]
...
# instantaneously
    ...
    def getValvePosition(self, deviceId):
        return self._device.valvePos[deviceId - 1]

    def getSyringePosition(self, deviceId):
    ...
    def getSyringe(self, deviceId=-1, syringetype=None,
                   maxvolume=None, **keyargs):
    ...
    def getValve(self, deviceId=-1, valvetype=None, **keyargs):
...
# ---------------------------------END of FAST REQUESTS-----------------------
    ...
    def initSyringeFromEncoder(self, deviceId, **keyargs):
